// ucs-api.workflow.js — 逐模块 UCS 生成 → 6 维安全审查 → 修正（三阶段 pipeline）
// 由 /simple:ucs-api 调用：主进程 Glob 定位本文件得绝对路径，传 scriptPath + args。
// args = { modules:[...], apiDir, ucsDir, reviewDir, date?, actions?, reviewActions?, templates:{ucs, ucsReview} }
//   actions:       { module: 'gen'|'skip' }  阶段1 闸门（UCS 文件已存在时用户的决定）
//   reviewActions: { module: 'gen'|'skip' }  阶段2 闸门（审查文件已存在时用户的决定）；skip 则跳过 review+fix
export const meta = {
  name: 'ucs-api-workflow',
  description: '逐模块 UCS 生成 → 6 维安全审查 → 修正（pipeline）',
  phases: [{ title: '生成' }, { title: '审查' }, { title: '修正' }],
}

// ---- args 校验 ----
const required = ['modules', 'apiDir', 'ucsDir', 'reviewDir', 'templates']
const missing = required.filter((k) => args[k] === undefined || args[k] === '')
if (missing.length) return { error: `ucs-api-workflow args 缺失: ${missing.join(', ')}` }
...
const GEN_RESULT = {
  ...
  properties: {
    ...
    error_codes_used: { type: 'array', items: { type: 'string' } },
  },
...
function genPrompt(module) {
  return `你是接口用例规约工程师。为模块「${module}」生成用例规约（UCS）文档，**只生成文档，不改任何接口定义**。

## 输入（读这些文件）
- 接口定义：${args.apiDir}/${module}.yaml（本模块全部接口）
- 数据库设计（存在才读）：docs/specs/data/ 下 DB 文件（字段名/类型以此为准）
- 技术选型：docs/standards/tech-stack-rule.md
- 目录结构：docs/standards/directory-rule.md
- HTTP 流转规范（存在才读）：docs/standards/http-handler-rule.md

## 生成要求
严格按下方「## UCS 结构模板」逐节填写：一、模块总览（1.1 用例清单 / 1.2 错误码号段）→ 二~N 各用例（每个接口至少一个用例，10 小节齐全，空节删除不留空壳）→ N+1 模块级技术要点汇总（Y.1~Y.8）。字段名/类型对齐 DB，流程对齐 http-handler-rule，使用中文。
...
2. **返回**（StructuredOutput）：{ module, path, use_case_count, error_codes_used }。
...
function reviewPrompt(module) {
  return `你是安全审查工程师。审查模块「${module}」的 UCS 文档，**只报告，不改 UCS**。

## 输入（读这些文件）
- 该 UCS：${args.ucsDir}/${module}.md
- 对应接口定义：${args.apiDir}/${module}.yaml
...
function fixPrompt(module) {
  return `你是用例规约修正工程师。按审查报告修正模块「${module}」的 UCS，**只改 UCS 文档**。

## 输入（读这些文件）
- 审查报告：${args.reviewDir}/${module}.md
- 对应 UCS：${args.ucsDir}/${module}.md
- 接口定义（必要时读）：${args.apiDir}/${module}.yaml
