...
## prompt 模板
    ...
    为你的 UCS 生成对应的 gRPC 测试代码。**先读 tech-stack-rule.md 确认语言、测试框架、依赖管理、编译命令**。
    ...
    | 语言 | 单元 | Mock | 集成/DB | E2E/gRPC |
    | Go | testify、go-cmp | go-sqlmock、go.uber.org/mock | testcontainers-go | bufconn（进程内 gRPC server）+ grpc 客户端 |
    | Node/TS | vitest、jest | 内置 mock、msw | testcontainers-node、pg-mem | @grpc/grpc-js 客户端 + 进程内 server |
    | Rust | 内置 + assert | mockall | testcontainers-rs、sqlx test | tonic + tower（进程内） |
    | Python | pytest | unittest.mock | testcontainers-python | grpc 测试 server + grpc.aio 客户端 |

    ## 测试文件与命名规范（按所选语言惯例）

    | 语言 | 文件命名（与源码同目录） | 函数/用例组织 |
    | Go | 集成 `*_integration_test.go`、E2E `*_e2e_test.go`、单元 `*_test.go` | `Test<Service>_<UCS编号>_<场景>`，表驱动 + t.Run |
    ...
    - grpc server（进程内启动 gRPC server，挂载拦截器；Go 用 bufconn，其他语言按生态）
