---
name: do-grpc
description: 按 grpc-UCS 实现后端 gRPC 服务代码并编写测试（执行型，仅后端，subagent 会写真实代码）。主流程只做编排：盘点 docs/specs/grpc-UCS 数量、控制待办；两阶段——先顺序逐一 subagent 实现所有 UCS（service/RPC 对齐 proto、数据模型对齐 DB、按需 tools、单任务编译通过），再顺序逐一 subagent 为所有 UCS 编写测试（从 UCS 提取场景→选测试类型→命名规范→覆盖矩阵→编译通过）；最后整体编译并更新测试脚本。当用户要实现 gRPC 服务代码或写测试时使用。
disable-model-invocation: true
---

# do-grpc

按 grpc-UCS 实现后端 gRPC 服务代码 + 编写测试。**执行型——subagent 在项目里写真实代码。仅后端适用。两阶段：先全部实现，再全部写测试。**
...
- 必须存在：`docs/specs/grpc-UCS/`（≥1 个 UCS）。
- 建议存在：`docs/specs/grpc/`（proto，RPC 实现对齐）、项目骨架已搭建（`do-directory` 产出）。
- 缺失必选项时，提示先运行对应 skill，结束。

## Step 1 — 盘点 UCS，生成待办/任务清单（主流程只做编排）

- 列出 `docs/specs/grpc-UCS/` 下所有 UCS 文件，**报告总数**。
...
按任务清单**顺序**逐一执行：每起一个 subagent 实现一个 UCS；该任务完成（subagent 报告编译通过）后再处理下一个。**不要并行、不要跳跃**——service 注册、依赖清单、数据模型等**共享文件**（具体文件名以所选技术栈 / 目录结构为准，不预设为某语言）靠顺序执行避免冲突。

每个 subagent 的 prompt 必须**自包含**（用 `templates/grpc-impl-prompt.md`，Glob 定位）：
- **由 subagent 自己读**：本 UCS + tech-stack-rule（语言/gRPC 框架/编译命令）+ directory-rule + tools-rule + docs/specs/data（含 struct.md 数据结构定义，如存在）+ 同名 proto。
- 实现该 UCS 所有 RPC（service 方法、参数校验、业务逻辑、响应封装）；service/RPC 对齐 proto；数据模型对齐 DB；按需开发 tools；**单任务编译通过**。
...
每个 subagent 的 prompt 必须**自包含**（用 `templates/grpc-test-impl-prompt.md`，Glob 定位）：
- **由 subagent 自己读**：本 UCS + tech-stack-rule（语言/测试框架）+ directory-rule + tools-rule + docs/specs/data（含 struct.md，如存在）+ 同名 proto + 现有源码（按 directory-rule 定位）。
