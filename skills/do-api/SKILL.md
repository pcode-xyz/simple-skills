...
- 为每个待处理 UCS 生成一个**待办/任务**：`任务N：<模块>.md → 实现 + 测试`，得到**任务清单**。
- **用待办事项分别登记并跟进**每个任务。
- 确认目标项目根目录（默认当前工作目录；`do-directory` 骨架所在）。

## Phase 1 — 逐任务实现（每任务一个 subagent，直接写码）
...
## Phase 2 — 逐任务写测试（每任务一个 subagent，直接写测试）
...
- 提示：可跑 `make test-unit`（或对应脚本）验证，或继续 `ucs-api`/`ucs-page` 下一轮迭代。
