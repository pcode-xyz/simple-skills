---
name: specs-design
description: 设计元素提取。按 design.md 规范（Google Labs 设计系统描述格式），从 docs/product/demo 的页面与 css/style.css 设计令牌中提取设计元素（色板/字体/字号/间距/圆角/阴影/组件），输出 DESIGN.md 到 docs/specs/design/。当用户要做设计系统提取、设计规范文档、DESIGN.md、design tokens 抽取时使用。
...
- 必须存在：`docs/product/demo/`（≥1 个 HTML 页面）、`docs/product/demo/css/style.css`（设计令牌基座）。
...
- 读 `docs/product/demo/css/style.css`：`:root` 设计令牌（色板/字体栈/字号阶梯/间距/圆角/阴影/描边）与共享组件类。
- 列出 `docs/product/demo/` 下所有页面 HTML，**报告总数**（不含 index.html 对比壳页）。
- 读 `docs/product/sense.md`：品牌人格、目标受众、情绪基调（Overview 参考）。

## Step 2 — 确认生成方式（AskUserQuestion）

- **单一 DESIGN.md**（推荐）：全站一套设计系统 → `docs/specs/design/DESIGN.md`。适合共享令牌集中在 css/style.css 的 demo。
...
- 起一个 subagent 生成整套设计系统文档；prompt 必须**自包含**（用 `templates/design-md-prompt.md`，Glob 定位）：
  1. **要读的文件**：`docs/product/demo/css/style.css`、`docs/product/demo/` 下全部页面 HTML（不含 index.html 对比壳页）、`docs/product/sense.md`。
  2. **生成要求**：按模板产出完整 DESIGN.md——frontmatter（colors/typography/rounded/spacing/components）＋ 正文相关节（Overview/Colors/Typography/Layout/Elevation & Depth/Shapes/Components/Do's and Don'ts，按序）；令牌从 css/style.css 提取、组件从页面提取、中文 prose、令牌与 prose 对应。
...
- 每个 subagent 的 prompt 必须**自包含**（用模板）：
  1. **要读的文件**：该页 HTML（只读这一页）＋ `docs/product/demo/css/style.css`（共享令牌基座）＋ `docs/product/sense.md`。
  2. **生成要求**：按模板产出该页 DESIGN.md——共享令牌基座从 css/style.css 提取，页面特有组件/样式归入 components，Overview 描述该页语境。
