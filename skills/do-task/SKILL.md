...
- **由 subagent 自己读**：本 task-UCS + tech-stack-rule（语言/编译命令）+ directory-rule + task-layer-rule + tools-rule + docs/specs/data（含 struct.md，如存在）+ 相关 API + 现有任务层源码（位置按 directory-rule / task-layer-rule）。
...
- **由 subagent 自己读**：本 task-UCS + tech-stack-rule（语言/测试框架）+ directory-rule + task-layer-rule + task-UCS 转测试提取规范（自带模板 `templates/task-ucs-to-testing-rule.md`，必须通读）+ docs/specs/data（含 struct.md，如存在）+ 现有任务层源码与测试（位置按 directory-rule / task-layer-rule）。
